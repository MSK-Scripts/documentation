...
- **`Config.progressBar` uses the table form of the progress bar.** The table form waits until the bar has run out, the old one returned right away. The bar runs in its own thread, so everything takes exactly as long as before.
