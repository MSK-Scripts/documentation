...
- **`Config.Commands.ausreise.startMoney`.** The balance `clear_money` sets is
  ...
  :::info[Add this to your config]
  Without the block `clear_money` does nothing instead of failing, so an old
  config file still starts. It just stops clearing the money.
  :::
...
- **`/raus` teleported into nowhere** when no position had been stored, for
  example after a resource restart. It falls back to the configured location now
  instead of sending a `nil` to the client.
...
- **The logout and drop handlers indexed a player that was already gone**, which
  ...
  drifted upward over a session. `playerDropped` is also a FiveM event now, not a
  framework one, and it takes only the reason.
