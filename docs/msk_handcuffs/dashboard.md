...
and checked at runtime. The group must therefore exist as a principal in your
`server.cfg`:
...
(since **v3.0.1**) through msk_core (**v3.3.0** or newer). That means your `server.cfg`
...
:::info[QBCore]
ESX names its group principals `group.<name>`, QBCore uses `qbcore.<name>` and its
staff levels are `god`, `admin` and `mod`. Since **v3.0.1** both spellings are
accepted, so a player set up with `/addpermission 1 mod` is recognised as group
`mod` just like one set up with `add_principal ... group.mod`.
:::
