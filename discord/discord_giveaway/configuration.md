...
| `bonus <role> <amount>` | role + `1`–`100` | — | Extra entries granted to a role for a weighted draw |
...
| `bonus <role>` | role | Remove the bonus entries for a role |
| `manager <role>` | role | Clear the manager role |
| `notify <role>` | role | Clear the notify role |

---

### Per-giveaway blacklist / whitelist

`blacklist` and `whitelist` (both `set` and `remove`) accept an **optional `giveaway_id`**:

- **Without** `giveaway_id` → the role is added to / removed from the **server-wide** list (applies to all giveaways).
- **With** `giveaway_id` → the role applies **only to that one giveaway**, *in addition* to the server-wide lists.

Eligibility is always evaluated against the **union** of the server-wide list and the giveaway's own list.
...
/gsettings remove blacklist role:@Muted giveaway_id:A1B2C3
```
